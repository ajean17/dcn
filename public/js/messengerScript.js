...
function sendmsg()
{
  ...
  if (message != "")
  {
    ...
    {
      if (xmlhttp.readyState==4 && xmlhttp.status==200)//When a request is sent and we get an answer from the server
      {
    ...
    xmlhttp.open("GET","/update?username=" + username + "&message=" + message,true);
    xmlhttp.send();
  }
...
function update()
{
  ...
  var output = "";
    ...
    xmlhttp.open("GET","getM?username=" + username,true);
    xmlhttp.send();
}
